"""SBERT semantic retrieval model for traceability ranking."""
...
def run_sbert(req_docs: Dict[str, str], code_docs: Dict[str, str]) -> Tuple[Dict[str, List[str]], np.ndarray, List[str], List[str]]:
    """Run SBERT retrieval and produce rankings.
    ...
    """
    logger.info("Model being executed: sbert")

    try:
        model = SentenceTransformer(EMBEDDING_MODEL_NAMES["sbert"])
    except Exception as exc:
        raise RuntimeError(f"Failed model loading: SBERT ({exc})") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Failed model loading: SBERT encoding ({exc})") from exc
